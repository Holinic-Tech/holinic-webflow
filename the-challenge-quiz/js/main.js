...
function handleRadioChangeChallenge(event, num) {
    ...
    const yes_result = document.getElementById("yes-result");
    const no_result = document.getElementById("no-result");
    const maybe_result = document.getElementById("maybe-result");

    // Show the right result
    if (selectedLabel == '👍 Yes') {
        yes_result.style.display = "block";
        no_result.style.display = "none";
        maybe_result.style.display = "none";
    } else if (selectedLabel == '❌ No') {
        yes_result.style.display = "none";
